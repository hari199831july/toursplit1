...
function render() {
  ...
  $('#balanceList').innerHTML = summary.balances.length
    ...
    : '<p class="muted empty-line">Add members to see balances.</p>';
  ...
  $('#settlementTable').innerHTML = transfers.length
    ? `
      <div class="settlement-head">
        <span>Member paying</span>
        <span>Member receiving</span>
        <span>Amount</span>
      </div>
      ${transfers.map(t => `
  ...
  $('#expenseList').innerHTML = state.expenses.length
    ? `<div class="expense-header"><span>Description</span><span>Paid by</span><span>Amount</span><span></span></div>` + state.expenses.map(e => {
    ...
    : '<p class="muted empty-line">No expenses added yet.</p>';
  ...
  $('#memberList').innerHTML = state.members.length
    ...
    `).join('')
    : '<p class="muted empty-line">No members added yet.</p>';
}
  ...
  return `
    ...
    <div class="form-grid">
      <div class="field">
        ...
        <input name="start" type="date" value="${esc(t.start)}">
      </div>
      <div class="field">
        <label>End date</label>
        <input name="end" type="date" value="${esc(t.end)}">
      </div>
    </div>
  `;
}
...
function expenseForm(e = {}) {
  ...
  return `
    ...
    <div class="form-grid">
      <div class="field">
        <label>Amount *</label>
        <input name="amount" required type="number" min="0.01" step="0.01" value="${esc(e.amount)}">
      </div>
      <div class="field">
        <label>Paid by *</label>
    ...
    <div class="field">
      <label>Date</label>
      <input name="date" type="date" value="${esc(e.date || new Date().toISOString().slice(0, 10))}">
    </div>
  `;
}
...
window.deleteMember = function(id) {
  if (confirm('Delete this member? Expenses paid by them will remain as Unknown.')) {
    ...
    state.expenses.forEach(e => {
      if (e.paidBy === id) e.paidBy = 'unknown';
    });
    save();
